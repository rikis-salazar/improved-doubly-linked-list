...
int main(){
  ...
  // Then verify that the stored values are 
  ...
  // Do not worry if you do not know about exceptions
  // we will not use them in this course. 
  // I decided to use them here because it is fairly
  // easy to figure out more or less how they work
  bool passed = false;
  try {
    *--list1.begin() = 0;  // Trying to dereference head->prev 
  } 
  ...
  // fails because an exception other than std::invalid_argument 
  // is thrown instead.
  if (!passed)
  ...
  passed = false;
  try {
    *list1.end() = 0; // Trying to dereference tail->next 
  } 
  catch (std::invalid_argument& ex) {
    passed = true;
